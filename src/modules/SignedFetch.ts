import { type SignedFetchRequest, signedFetch, getHeaders } from '~system/SignedFetch'
...
export function create(): Record<string, any> {
  async function internalSignedFetch(url: string, init?: FlatFetchInit): Promise<FlatFetchResponse> {
  ...
  return {
    ...
    async getHeaders(request: SignedFetchRequest) {
      return await getHeaders(request)
    }
  }
}

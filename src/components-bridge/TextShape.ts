...
function update(state: AdaptationLayerState, ecs6EntityId: EntityID, payload: ECS6ComponentTextShape): void {
  ...
  value.fontSize = payload.fontSize === undefined ? 10 : payload.fontSize * 0.75
  value.font = convertFont(state, payload.font)
  const lineSpacing = Number.parseFloat(payload.lineSpacing ?? '0')
  value.lineSpacing = Number.isFinite(lineSpacing) ? lineSpacing : 0
  ...
  value.textAlign = stringToTextAlignMode(textAlignFromHV(payload.hTextAlign, payload.vTextAlign))

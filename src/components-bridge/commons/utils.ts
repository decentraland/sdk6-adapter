import { videoEntity } from '../VideoTexture'
import { ColliderLayer, InputAction, Material, TextureWrapMode, type TextureUnion } from '@dcl/ecs'

import { ECS6_ActionButton, ECS6_CLASS_ID, type AdaptationLayerState } from '../../types'
import { Font } from '@dcl/sdk/ecs'
import { type ECS6ComponentAudioClip, type ECS6ComponentFont } from '~system/EngineApi'

export function getColliderLayer(payload: any): number | undefined {
  let mask = ColliderLayer.CL_NONE
  if (payload.isPointerBlocker !== false) mask |= ColliderLayer.CL_POINTER
  if (payload.withCollisions !== false) mask |= ColliderLayer.CL_PHYSICS
  return mask === ColliderLayer.CL_NONE ? undefined : mask
}
...
export function convertTexture(state: AdaptationLayerState, textureEntityId: any): TextureUnion | undefined {
  if (textureEntityId !== undefined) {
    ...
    if (!texturePayload) return undefined
    switch (textureData.classId) {
      ...
      case ECS6_CLASS_ID.VIDEO_TEXTURE: {
        const entity = videoEntity(textureEntityId)
        return entity === undefined ? undefined : Material.Texture.Video({ videoPlayerEntity: entity })
      }
    }
  }
  return undefined
...
export function convertFont(state: AdaptationLayerState, fontEntityId: any): Font | undefined {
  if (fontEntityId !== undefined) {
    ...
    if (!fontPayload) return undefined

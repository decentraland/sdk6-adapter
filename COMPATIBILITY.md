...
| GLTF, NFT, animation, billboard and TextShape component mapping | FULL | SDK7 component writes and resource references; TextShape fields follow the legacy renderer ([docs/class21.md](docs/class21.md)) and an unset text value is empty text. See visual limitations below. |
...
| Screen UI component mapping | FULL | Canvas, rectangles, stacks, text, images and input; fullscreen aliases canvas. Legacy rules kept: a shape blocks and receives the pointer only while it and every ancestor are visible and `isPointerBlocker`; `onClick` fires once per pointer down with `{ buttonId: 0 }`; images crop to the source rect whenever the texture size is known (`sizeInPixels: false` scales it by the parent rect); stacks adapt their size by default; rect `thickness` draws the half-transparent black outline; text font sizes truncate and `fontAutoSize` disables wrapping; inputs colour text with `placeholderColor`, paint `focusedBackground`, drop empty submissions and route SDK6 6.x `onTextChanged` as the library expects. |
...
| Host-supplied player/realm/scene/comms events | FULL | Forwarded once per host event; held until the scene's start-up Identity calls settle (capped). |
...
| Unknown legacy modules | FULL | `loadModule` rejects unknown names as the legacy runtime did; the loader stays silent on a host-module miss and reports other loader errors. |
...
| [Legacy orphan-UI fallback to an existing screen canvas](docs/class23.md) | FULL | Missing-parent children render under the scene's first screen-space canvas as in the legacy renderer; hidden without one. |
...
| GLTF/NFT/animation/billboard/TextShape visual parity | PARTIAL | Renderer fidelity varies; TextShape outline, shadow and line layout depend on the host. Legacy skinned colliders may require external [asset repair](docs/collider-skins.md). |
...
| Screen UI fonts, outlines, autosizing and complex layout | PARTIAL | Historical inset remains; typography and layout differ. SDK7 UI carries no text `outlineWidth`, `outlineColor`, `lineSpacing`, `lineCount`, `fontWeight`, shadow fields or `adaptWidth`/`adaptHeight`, no input `onFocus`/`onBlur` and no right-button clicks (`onClick` answers the pointer button only); the input `color` field is ignored as the legacy renderer painted `placeholderColor` over it. |
| UiImage source-rect cropping | PARTIAL | Requires the host's optional `~system/AdaptationLayerHelper.getTextureSize`; where the host does not provide it or the lookup fails, the image renders the whole texture instead of the source rect and pixel sizing derived from the texture's natural size is skipped. |
